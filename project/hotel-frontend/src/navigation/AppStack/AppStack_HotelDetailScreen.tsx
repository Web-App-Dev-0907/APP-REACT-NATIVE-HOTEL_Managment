...
import {Coffee, CoffeeBlack, Group, Swimming} from '../../lib/images';
...
const AppStack_HotelDetailScreen: React.FC<Props> = ({navigation, route}) => {
  ...
  const onPressReserve = () => {
    const checkIfRoomSelected = rooms.find(room => room.selected);
  ...
  const onPressPurchase = async () => {
    const response = await http.post('/create-payment-intent', {
      amount:
        rooms.find(room => room.selected).price *
    ...
    if (error) {
      ...
      if (error) {
        ...
        http
          .post('/user/reserve_room', {
            userId: user._id,
            roomId: rooms.find(room => room.selected)._id,
            hotelId: route.params.item._id,
            paymentIntentId,
          ...
          .then(res => {
            setSelectedDates([]);
            setRooms((prev: any) =>
              prev.map((room: any) => ({...room, selected: false})),
            );
          });
      }
    }
      ...
      <ScrollView>
        ...
        <View style={tw`px-2`}>
          ...
          <View style={tw`flex-row mb-1.5 items-center`}>
            {route.params.item.amenities.map((amenity: any, index: number) => (
          ...
          <View style={tw`ml-6`}>
            {route.params.item.checkIn.map((checkIn: any, index: number) => (
              <Text
                key={index}
          ...
          <View style={tw`ml-6`}>
            {route.params.item.checkOut.map((checkout: any, index: number) => (
              <Text
                key={index}
                style={tw`text-black font-dm text-[8px] font-bold mb-1.5`}>
                {checkout.name}
        ...
        <View
          style={tw`py-3 bg-[#1E2761] w-full flex-row ${
            selectedDates.length > 0 && rooms.some(room => room.selected)
              ? 'justify-between'
              : 'justify-end'
          }`}>
          {selectedDates.length > 0 && rooms.some(room => room.selected) && (
            <View style={tw`pt-4 px-8 flex-1`}>
              ...
              <Text style={tw`text-white font-dm text-[10px]`}>{`Tax & Fees: $${
                (rooms.find(room => room.selected).price *
                  selectedDates.length *
